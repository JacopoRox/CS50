...
function Player:update(dt)
    if self:checkVictory() then
        self.x = map.flag.x - self.width / 2
        WALKING_SPEED = 0
        if self.y < self.map.tileHeight * ((map.mapHeight - 2) / 2) - self.height then
            self.dy = map.flag.dy
        end
    end
...
function Player:checkVictory()
    if self.x + self.width >= map.flag.x and self.y + self.height >= map.flag.y then
        self.victory = true
    end
    return self.victory
end
